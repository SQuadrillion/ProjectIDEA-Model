...
namespace ProjectIDEAModel
{
    ...
    {
        string URL = "https://peaceful-badlands-44972.herokuapp.com";          public async Task<IEnumerable<Song>> GetSongsAsync()         {             var res = await GetRequestAsync("/api/songs");             var songObjects = JsonConvert.DeserializeObject<IEnumerable<SongObject>>(res);             return songObjects.Select(x => Convert(x));         }
         public async Task<IEnumerable<Item>> GetItemsAsync()         {             var res = await GetRequestAsync("/api/queues");             var itemObjects = JsonConvert.DeserializeObject<IEnumerable<ItemObject>>(res);             return itemObjects.Select(x => Convert(x));         }          public async Task<Item> GetPlayingAsync()         {             var res = await GetRequestAsync("/api/playing");             var itemObject = JsonConvert.DeserializeObject<ItemObject>(res);             return Convert(itemObject);         }          public async Task<string> PostItemAsync(int songId, string userName)         {             return await PostRequestAsync("/api/queues", "{\"id\":" + songId + ",\"name\":\"" + userName + "\"}");         } 
